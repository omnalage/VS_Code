...
def run_simulation(routers, publishers, subscribers, policy, iterations):
    """Run the simulation for the specified caching policy and save datasets."""
    ...
    for _ in range(iterations):
        ...
        if active_subscribers:
            ...
            # Create and send interest packet
            ...
            # Track the actual hop count after receiving data
        ...
        # Collect feedback data for this iteration (randomized feedback for now)
        feedback_scores = [random.choice(['like', 'dislike', 'neutral', 'highly_like', 'highly_dislike']) for _ in subscribers]

        # Save this iteration's data
        simulation_data.append([
            datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),   # Time of the simulation run
            len(active_subscribers),        # Number of active clients (subscribers)
            total_requests,                 # Total requests made in this iteration
            total_hop_reduction,           # Average hop reduction (efficiency)
            avg_cache_hit,                 # Cache hit ratio
            avg_latency,                   # Latency
            feedback_scores                # List of feedback scores from subscribers
        ])

    # Save the simulation data for this policy
    save_simulation_data(simulation_data, policy)
...
def run_simulation_for_all_policies(routers, publishers, subscribers, iterations):
    ...
    for policy in policies:
        ...
        data_for_policy = run_simulation(routers, publishers, subscribers, policy, iterations)
...
def main():
    ...
    # Run the simulation for all policies and save datasets
    all_simulation_data = run_simulation_for_all_policies(routers, publishers, subscribers, iterations)
