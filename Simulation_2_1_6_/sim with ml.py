...
def predict_policy(features):
    """Predict the best policy based on network features using the trained Random Forest model."""
...
def run_simulation(routers, publishers, subscribers, iterations, rf_model):
    ...
    for _ in range(iterations):
        ...
        if active_subscribers:
            ...
            interest_packet.original_hop_count = len(routers)  # Estimate max possible hops
        ...
        # Use the trained Random Forest model to predict the best policy
        network_features = [avg_cache_hit, avg_latency, total_requests]
        predicted_policy = rf_model.predict([network_features])[0]
        print(f"Predicted Policy for this iteration: {predicted_policy}")

        # Apply the predicted policy to the router for this iteration
        for router in routers:
            router.caching_policy = predicted_policy

        # Save the simulation data (include the predicted policy)
        simulation_data.append([
            ...
            avg_cache_hit,
            avg_latency,
            predicted_policy  # Log the predicted policy used
        ])

    save_policy_stats(predicted_policy, simulation_data)  # Save the simulation data for the predicted policy
    plot_simulation_log(simulation_data, predicted_policy)
...
def save_simulation_log(simulation_data):
    ...
    with open('Simulation_Log/simulation_log.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Simulation Time", "No of Clients", "Total no of requests", "Average hop reduction", "Average Cache hit", "Average latency", "Policy"])
...
def plot_merged_graph(policy_stats):
    """Plot a merged graph comparing all traditional caching policies and the ML-based policy."""
    df = pd.DataFrame(policy_stats)
    ...
    traditional_colors = {
        ...
        'MRU': 'indigo',
        'FACR': 'saddlebrown',
        'ML_Policy': 'red'  # Color for the ML-based policy
    }
    ...
    plt.suptitle("Comparison of Caching Policies: LRU, LFU, FIFO, MRU, FACR, and ML_Policy", fontsize=14, fontweight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.show()
...
def main():
    ...
    try:
        iterations = int(input("Enter the number of content requests in the simulation: "))  # Convert to integer
    except ValueError:
    ...
    for policy in policies:
        ...
        # Make sure iterations is passed as an integer to the simulation function
        stats = run_simulation(routers, publishers, subscribers, policy, int(iterations))
